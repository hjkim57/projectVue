      ...
      <ul class="information_2">
          ...
          <fieldset class="names_c">
            <label v-for="mem in copyCustomerData" :key="mem.cus_id">
                <template v-if="mem.m_id == myPick_manager">
                    <input type="radio" name="customers" :value="mem.cus_id" v-model="myPick_cus" />
                    {{ mem.cus_name }} <br>
                </template>
            </label>

          <!-- <label v-for="mem in copyCustomerData" :key="mem.cus_id" v-if="mem.m_id == myPick_manager" >
          <input v-if="mem.m_id == myPick_manager" type="radio" name="customers" :value="mem.cus_id" v-model="myPick_cus" />
          {{ mem.cus_name }}<br>
          </label> -->
          </fieldset>
          <!-- <div v-for="mem in localMember" :key="mem">             -->
...
computed: {
  localMember() {
    ...
    if (Array.isArray(this.copyCustomerData)) {
      return this.copyCustomerData.find(customer => customer.cus_id === this.myPick_cus) || {};
    }
  ...
  selectedCustomer() {
  // Ensure copyCustomerData is defined and is an array
    if (Array.isArray(this.localMember)) {
      return this.localMember.find(customer => customer.cus_id === this.myPick_cus) || {};
    }
    return {}; // Return an empty object if copyCustomerData is not an array
  }
},
...
methods: {
  ...
  setDefaultCustomer() {
    const firstCustomer = this.copyCustomerData.find(customer => customer.m_id === this.myPick_manager);
    if (firstCustomer) {
      this.myPick_cus = firstCustomer.cus_id;
    }
  },
  mounted() {
